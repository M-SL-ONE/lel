...
$(function(){
	...
	$('.testServer').click(function(){
		if ($('.target').length==1) {
			...
			$('.testInfo').parent().css('opacity','1')
			// if (port==site) {
				var url = site+'/test.ashx?oper=test&mobile='+mobile+'&user='+'&pass='+'&'+time;
				// alert(url)
			...
			// } else {
			// 	$('.testInfo').parent().css('background','red');
			// 	$('.testInfo').attr('value','false');
			// 	$('.config-list').css('height',$(window).height() - $('.header').height()- $('.config-target').outerHeight(true) - $('.configCode').height())
			// }
	...
	$('.changeName').change(function(){
		$('.editName').attr('value',$('.changeName').val());
	})
	...
	$('.saveServer').click(function(){
		var data;
		ajax('get','http://lyl.hcpos.com/freedom/lelcom.ashx?oper=6&mobile='+mobile+'&shopname='+$('.editName').val()+'&agent='+$('.editDls').val()+'&softver='+bb,'',function(data) {	
			data = JSON.parse( data );
			if (data.success=='true') {
				if (confirmServer()) {
					$('.serverEdit').hide();
					...
					var tr='<div class="clearfix wrap">'+
							...
							'</div>'+
						'</div>';
					var option = '<option site="">'+$('.editName').val()+'</option>'
					$('#server').apppend(tr);
